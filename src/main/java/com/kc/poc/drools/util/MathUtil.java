...
public abstract class MathUtil {
    ...
    /**
     * Si la `value` == 0, alors retourner null, sinon retourner la `value`
     */
    public static BigDecimal nullify(BigDecimal value) {
        if (value == null) {
            return null;
        }

        if (equalsIgnoreScale(value, BigDecimal.ZERO)) {
            return null;
        }
    ...
    public static <T> BigDecimal addProperties(Collection<T> collection, Function<T, BigDecimal> getter) {
        return add(collection.stream().map(getter));
    }

    public static BigDecimal add(Stream<BigDecimal> stream) {
	...
	public static BigDecimal divide(BigDecimal number, BigDecimal divisor, int scale) {
		return round(
            MathUtil.divide(number, divisor),
            scale
        );
	}
	...
	public static BigDecimal divide1000(BigDecimal value) {
    	return divide(denullify(value), BigDecimal.valueOf(1000));
    }
    ...
    public static Map<Integer, BigDecimal> toValueBigDecimal(Map<Integer, String> map) {
        if(map == null)
        {
            return new HashMap<>();
        }
        return map.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e -> new BigDecimal(e.getValue())));
    }

    public static boolean equalsIgnoreScale(BigDecimal val1, BigDecimal val2) {
        if (val1 == null) {
